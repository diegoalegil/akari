...
export async function buildDb(manifest: Manifest): Promise<Record<string, number>> {
  ...
  // NOTE: the user's in-app Anthropic key is intentionally NOT carried into the
  // rebuilt seed — the shipped public/akari.db.gz must never contain a secret.
  // An existing user's key survives a content update on the client, where the
  // upgrade copies the settings table over (lib/clientDb.ts copyTable).
  ...
  let jmHits = 0;
  const wordsRaw: WordRow[] = notes.map((n) => {
  ...
  // Dedupe identical words: Kaishi occasionally ships the same (expression,reading)
  // twice, which would otherwise drill as two separate cards. In kaishi_order, keep
  // the first occurrence; drop a later duplicate ONLY when its meaning_en matches the
  // kept row's — genuinely-different-meaning homographs stay as separate cards.
  const keptByPair = new Map<string, WordRow>();
  const words: WordRow[] = [];
  for (const w of [...wordsRaw].sort((a, b) => a.order - b.order)) {
    const key = w.expression + "\x00" + w.reading;
    const kept = keptByPair.get(key);
    if (kept && kept.meaning === w.meaning) continue; // exact duplicate — skip
    if (!kept) keptByPair.set(key, w);
    words.push(w);
  }

  // jmHits was counted in the pre-dedup map; recompute it from the SURVIVING words
  // so the audit stats (seed-report.json, ingest_meta, the printed counts) match
  // the rows actually shipped (dedup drops a few jmdict-matched duplicates).
  jmHits = words.filter((w) => w.meaningSource === "jmdict").length;
  ...
  // Homograph guard: surfaces that map to >1 distinct reading across the deck.
  // Tatoeba is matched by raw surface substring (no reading check), so attaching
  // by bare surface to a homograph illustrates the WRONG reading. For these we
  // skip Tatoeba entirely — the guaranteed Kaishi bundle sentence still covers them.
  const readingsBySurface = new Map<string, Set<string>>();
  for (const w of words) {
    let s = readingsBySurface.get(w.expression);
    if (!s) readingsBySurface.set(w.expression, (s = new Set<string>()));
    s.add(w.reading);
  }
  const homographSurfaces = new Set<string>();
  for (const [surface, readings] of readingsBySurface) {
    if (readings.size > 1) homographSurfaces.add(surface);
  }
  ...
  for (const w of words) {
    ...
    // Homograph surfaces: skip Tatoeba (bare surface match can't tell readings apart).
    if (homographSurfaces.has(w.expression)) continue;

    // Short headwords match Tatoeba by bare substring with no morpheme/reading
    // boundary, so they collide with inflectional endings (こう in 行こう), particle
    // boundaries (もし in 何もしない) or larger compounds that read the kanji
    // DIFFERENTLY (中 as ちゅう in 中古, not なか) — teaching a wrong example or even
    // a wrong reading. These keep their validated rank-0 Kaishi example, so skip
    // Tatoeba for single-character surfaces and kana-only surfaces of length ≤ 2.
    const kanaOnly = !kanjiChars(w.expression).length;
    if (w.expression.length === 1 || (kanaOnly && w.expression.length <= 2)) continue;
    ...
    // silently cost the word an example via the (word_id,sentence_id) PK. Seed it
    // with the rank-0 Kaishi sentence too, so a Tatoeba pick whose JP is identical
    // isn't attached again (the learner would otherwise see the same sentence twice).
    const seenText = new Set<string>();
    const kaishiJp = notes[w.order - 1]?.sentenceJp;
    if (w.expression && kaishiJp) seenText.add(kaishiJp);
  ...
  // SRS surfaces: kana, words, and the kanji that appear in your vocabulary —
  // the latter studied by HANDWRITING (drawn stroke-by-stroke, matched against
  // the validated KanjiVG reference). Only kanji with a stroke SVG can be drawn,
  // so they gate the card set; inert kanji you'll never meet are left out.
  const writableKanji = kanjiRows.filter((k) => inVocab.has(k.literal) && !!k.svg);
  const cardSeeds: CardSeed[] = [
    ...kana.map((_, i) => ({ type: "kana" as const, id: i + 1 })),
    ...words.map((w) => ({ type: "word" as const, id: w.id })),
    ...writableKanji.map((k) => ({ type: "kanji" as const, id: k.id })),
  ];
